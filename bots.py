...
class RiskTakerKareem(BaseBot):
    """RiskTakerKareem always rolls again if it is more than one dice remaining""" 
    
    def _roll_bank_or_quit(self):
        if(self.dice_remaining>4):
            return "r"
        elif(self.dice_remaining >1 and self.unbanked_points<900):
          return "r"
        else:
            return "b"
        
        
class ReasonabelKid(BaseBot):
    """ReasonabelKid is a thinker he don't take that much risk but he is not a reserved too he roll again if the 
    remaining dice is more than two and the score is less than 500 Or if the remaining """
    def _roll_bank_or_quit(self):
        if(self.dice_remaining>4):
            return "r"     
        elif(self.dice_remaining>2 and self.unbanked_points<500):
            return "r"
        else:
            return "b"
